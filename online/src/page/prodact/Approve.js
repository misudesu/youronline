...
export default function Approve() {
  const [Appruved,setAppruved]=useState([]);
    ...
    useEffect(() => {
      ...
      const q = query(prodactRef,  where("Status", "==", "Appruved"));  onSnapshot(q, (snapshot) => {
...
//     const prodactRef=firestore().collection('prodact');
// const snapshot=prodactRef.orderBy('id').Limit(3).get();
// if(!snapshot.empty){
//   let newData=[];
//   for(let i=0;i<snapshot.docs.length;i++){
//     newData.push(snapshot.docs[i].data());
//   }
//   console.log(newData);
// }
  });
