...
export default function Update(props) {
 ...
 const appruve=(e)=>{
  const userinfo = doc(db, "Prodact",id);
  updateDoc(userinfo, {
    Status: "Appruved",
  });
  alert("The prdact is Approved")
  }
  const reject=(e)=>{
    const userinfo=doc(db,"Prodact",id)
    updateDoc(userinfo,{
...
<li>Address: <span>{data.address}</span></li>
  ...
  <div className='flex gap-x-4 m-3'>
    <button className='bg-blue-500 text-indigo-100 hover:bg-green-600 rounded-md py-1 px-8' onClick={appruve}>Appruve</button>
    <button className='bg-gray-600 text-indigo-100 hover:bg-red-600 rounded-md py-1 px-8' onClick={reject}>Reject</button>
  </div>
      </div>
      </>
  ))}
